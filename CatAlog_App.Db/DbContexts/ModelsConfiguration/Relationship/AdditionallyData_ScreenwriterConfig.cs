...
namespace CatAlog_App.Db.DbContexts.ModelsConfiguration.Relationship
{
    public class AdditionallyData_ScreenwriterConfig : IEntityTypeConfiguration<AdditionallyData_Screenwriter>
    {
        ...
        {
            builder.HasOne(a => a.AdditionallyData).WithMany(s => s.AdditionallyData_Screenwriters).HasForeignKey(k => k.AdditionallyInfoId).OnDelete(DeleteBehavior.Cascade);
            ...
            builder.HasData
                (
                    ...
                    {
                        new  AdditionallyData_Screenwriter() { Id = 1, AdditionallyInfoId = 4, ScreenwriterId = 1 },
                        new  AdditionallyData_Screenwriter() { Id = 2, AdditionallyInfoId = 6, ScreenwriterId = 1 }
                    }
                );
        }
    }
