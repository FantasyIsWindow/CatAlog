...
{
    public class ScreenshotConfig : IEntityTypeConfiguration<Screenshot>
    {
        public void Configure(EntityTypeBuilder<Screenshot> builder)
        {
            ...
            builder.HasOne(m => m.MainRecordData).WithMany(s => s.Screenshots).HasForeignKey(k => k.MainRecordDataId).OnDelete(DeleteBehavior.Cascade);
            builder.HasData
                (
                    new Screenshot[]
                    {
                        ...
                        new Screenshot() { Id = 20, Path = _20, MainRecordDataId = 6}
                    }
                );
        }
    }
